...
const PlantTaskDashboard: React.FC<PlantTaskDashboardProps> = ({ plantTasks }) => {
  ...
  const urgentTasks = plantTasks.filter(task => (task.taskUrgency === 'HIGH' || task.taskUrgency === 'IMMEDIATE') && task.taskStatus !== 'COMPLETED' && task.taskStatus !== 'CANCELLED').length;
  const overdueTasks = plantTasks.filter(task => moment(task.dueDate).isBefore(moment()) && task.taskStatus !== 'COMPLETED' && task.taskStatus !== 'CANCELLED').length;
  ...
  return (
    <>
      <Row gutter={16} className="mb-4">
        ...
        <Col span={6}>
          <Statistic title="Urgent Tasks" value={urgentTasks} />
        </Col>
        <Col span={6}>
          <Statistic title="Overdue Tasks" value={overdueTasks} />
        </Col>
        <Col span={6}>
          <Statistic title="Total Tasks" value={plantTasks.length} />
        </Col>
      </Row>
      <Row gutter={16}>
        <Col span={12}>
