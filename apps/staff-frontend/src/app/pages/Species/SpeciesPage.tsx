...
import { Descriptions, Card, Row, Col, Input, Tag, Flex, Button, Table, Modal, message, Tooltip } from 'antd';
...
const SpeciesPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string) => (
        <Tag bordered={false} color={text === 'LEAST_CONCERN' ? 'green' : 'red'}>
          {text}
        </Tag>
      ),
    },
  ...
  return webMode ? (
    ...
    <ContentWrapperDark>
      ...
      <Flex justify="end" gap={10}>
        <Search placeholder="Search species" allowClear enterButton="Search" onSearch={handleSearch} style={{ marginBottom: 20 }} />
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/species/create')}>
          Create Species
        </Button>
      </Flex>
      <Card bordered={false}>
        <Table
          ...
          onRow={(record) => ({
            onClick: () => navigateToSpecies(record.id),
            className: 'cursor-pointer',
          })}
          expandable={{
            expandedRowRender: (species) => {
              const descriptionsItems = Object.entries(species).map(([key, val]) => ({
                key,
              ...
              return (
                <div>
                  ...
                  <Button className="m-1" type="primary" onClick={() => handleEdit(species.id)}>
                    Edit
                  </Button>
                  <Button className="m-1" type="primary" danger onClick={() => handleDelete(species.id)}>
                    Delete
                  </Button>
                </div>
              );
