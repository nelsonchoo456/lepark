import {
  ...
  class EventTicketService {
    ...
    public async getEventTicketTransactionById(
      id: string,
    ...
    public async getEventTicketById(
      ...
      EventTicket & { eventTicketListing: EventTicketListing; eventTicketTransaction: EventTicketTransaction }
    > {
      ...
      const eventTicketTransaction = await EventTicketDao.getEventTicketTransactionById(
        updatedTicket.eventTicketTransactionId,
      );
    ...
    public async getEventTicketsByEventId(
      eventId: string,
    ): Promise<(EventTicket & { eventTicketListing: EventTicketListing })[]> {
      const event = await EventDao.getEventById(eventId);
      if (!event) {
        throw new Error('Event not found');
      }
    ...
    private async updateTicketStatusIfExpired(
      ...
      if (!ticket.eventTicketTransaction) {
        ticket.eventTicketTransaction = await EventTicketDao.getEventTicketTransactionById(
          ticket.eventTicketTransactionId,
        );
      }
