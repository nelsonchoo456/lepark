...
function getAugumentedDataset(data, m) {
  console.log('getAugumentedDataset called with data:', data, 'and m:', m);
  ...
  if (!prediction) {
    console.error('No valid prediction found');
    return;
  }
  ...
  // console.log('getAugumentedDataset returning:', {
...
function getForecast(data, alpha, beta, gamma, period, m) {
  //console.log('getForecast called with alpha:', alpha, 'beta:', beta, 'gamma:', gamma, 'period:', period, 'm:', m);
  var seasons, seasonal, st1, bt1;

  if (!validArgs(data, alpha, beta, gamma, period, m)) {
    //console.error('Invalid arguments for getForecast');
    return;
  }
