...
function getAugumentedDataset(data, m) {
  ...
  // Discard all negative values from augumentedDataset
  augumentedDataset = augumentedDataset.filter(value => value >= 0);

  // Ensure we have at least 5 predictions
  while (augumentedDataset.length < data.length + m) {
    augumentedDataset.push(augumentedDataset[augumentedDataset.length - 1]);
  }

  return {
    augumentedDataset: augumentedDataset.slice(-m),
    alpha: alpha,
    beta: beta,
    gamma: gamma,
    period: period,
