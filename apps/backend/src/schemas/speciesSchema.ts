...
export const SpeciesSchema = z.object({
  ...
  waterRequirement: z.number().int().min(0).max(100, { message: "Water requirement must be between 0 and 100" }),
  fertiliserRequirement: z.number().int().min(0).max(100, { message: "Fertiliser requirement must be between 0 and 100" }),
  idealHumidity: z.number().min(0).max(100, { message: "Ideal humidity must be between 0 and 100" }),
